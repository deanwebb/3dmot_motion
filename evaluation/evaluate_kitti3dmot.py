...
class trackingEvaluation(object):
    ...
    def createEvalDir(self):
        ...
        self.eval_dir = os.path.join("./results/", self.t_sha, "eval",
                                     self.cls)
    ...
    def compute3rdPartyMetrics(self, threshold=-10000):
            ...
            for f in range(len(seq_gt)):  # go through each frame
                ...
                for i, tt in enumerate(this_ids[0]):
                    ...
                    if tt in last_ids[0]:
                        ...
                        tid = this_ids[1][
                            i]  # id in current tracker corresponding to the gt tt
                        lid = last_ids[1][
                            idx]  # id in last frame tracker corresponding to the gt tt
        ...
        if self.n_gt != 0:
            if self.id_switches == 0:
                self.MOTAL = 1 - (
                    self.fn + self.fp + self.id_switches) / float(self.n_gt)
            else:
    ...
    def createSummary_details(self):
        ...
        summary += "evaluation: best results with single threshold".center(
            80, "=") + "\n"
        ...
        summary += self.printEntry(
            "Multiple Object Detection Precision (MODP)", float(
                self.MODP)) + "\n"
        summary += "\n"
    ...
    def createSummary_simple(self, threshold):
        ...
        summary += ("evaluation with confidence threshold %f" %
                    threshold).center(80, "=") + "\n"
...
class stat:
    ...
    def plot_over_recall(self, data_list, title, y_name, save_path):
        ...
        ax.plot(
            np.array(self.recall_list + extra_zero),
            np.array(data_list + y_zero))
        ...
        if y_name in ['MOTA', 'F1']:
            ...
            # print(max_ind)
            plt.axvline(
                self.recall_list[max_ind], ymax=data_list[max_ind], color='r')
            plt.plot(
                self.recall_list[max_ind],
                data_list[max_ind],
                'or',
                markersize=12)
            plt.text(
    ...
    def plot(self):
        ...
        self.plot_over_recall(
            self.mota_list, 'MOTA - Recall Curve', 'MOTA',
            os.path.join(
                save_dir,
                'MOTA_recall_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_recall(
            self.motp_list, 'MOTP - Recall Curve', 'MOTP',
            os.path.join(
                save_dir,
                'MOTP_recall_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_recall(
            self.f1_list, 'F1 - Recall Curve', 'F1',
            os.path.join(
                save_dir,
                'F1_recall_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_recall(
            self.fp_list, 'False Positive - Recall Curve', 'False Positive',
            os.path.join(
                save_dir,
                'FP_recall_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_recall(
            self.fn_list, 'False Negative - Recall Curve', 'False Negative',
            os.path.join(
                save_dir,
                'FN_recall_curve_%s_%s.pdf' % (self.cls, self.suffix)))
        self.plot_over_recall(
            self.precision_list, 'Precision - Recall Curve', 'Precision',
            os.path.join(
                save_dir,
                'precision_recall_curve_%s_%s.pdf' % (self.cls, self.suffix)))
...
def evaluate(result_sha, mail, eval_3diou, eval_2diou):
    ...
    for c in ("car", "pedestrian"):
        ...
        filename = os.path.join("./results", result_sha,
...
if __name__ == "__main__":
    ...
    if len(sys.argv) == 3:
        if sys.argv[2] == '2D':
            ...
            print(
                "Usage: python eval_kitti3dmot.py result_sha ?D(e.g. 2D or 3D)"
            )
            sys.exit(1)
    else:
        eval_3diou, eval_2diou = True, False  # eval 3d
